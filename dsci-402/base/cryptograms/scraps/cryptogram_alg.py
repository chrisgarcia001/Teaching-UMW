...
# Split raw text into clean (i.e. lowercase, non-alphabetical characters removed) tokens.
def tokenize(text):
	tokens = filter(lambda y: len(y) > 0, map(lambda x: x.lower().strip(), text))
...
def read_words(filename):
	return tokenize(open(filename).readlines())
...
# This is the main method - take encrypted message and word list, and return the decryption.
def solve_cryptogram(encrypted_message_file, word_list_file):
	...
	if(key != None):
		return ' '.join(map(lambda x: decode(x, key), encrypted_message)) + "\n"
	else:
		return 'No solution in given dictionary word list!'
